...
fig=figure;
%{
VySLD=uislider(fig,'position',[50 990 200 3],'limits',[1000 40000],'ValueChangedFc',@(hObject,eventdata)updateVyRef(hObject,eventdata));
VyLBL=uilabel('parent',fig,'Text','DESIRED VyITUDE','background','g','position',[50 945 200 15]);
VyGain=uislider(fig,'position',[50 935 200 3],'limits',[0 3],'ValueChangedFc',@(hObject,eventdata)updateVyGain(hObject,eventdata));
VyGainLBL=uilabel('parent',fig,'Text','VyITUDE GAIN','background','g','position',[50 890 200 15]);
...
elseif(it>=N && mod(it,M)==10)
    ...
    hLine2.YData=Vy(it-N+1:it);
   drawnow  %updates the display
end
Dpitch=[pitch(it) OldstatPitch]*[3 -4 1]';
OldstatPitch(2)=OldstatPitch(1);
OldstatPitch(1)=pitch(it);
DVy=[Vy(it) OldstatVy]*[3 -4 1]';
OldstatVy(2)=OldstatVy(1);
OldstatVy(1)=Vy(it);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pitchRef = KVy * (VyRef - Vy(it));
VyErInt = VyErInt + VyRef - Vy(it);
pitchRef = sat(pitchRef + KVyInt * VyErInt,5,-5);
pitch_trim = Kpitchrate * (Kpitch * (pitchRef - pitch(it)) - Dpitch);
pitch_trim = sat(elev,1,-1);
...
throttle_cmd=[throttle throttle No_command No_command No_command No_command No_command No_command];
primary_ctr_cmd=[0 No_command No_command No_command No_command No_command No_command No_command];
sec_ctr_cmd=[elev No_command No_command No_command No_command No_command No_command No_command];
...
function updateVyRef(~,~)
VyRef=VySLD.Value;
end
function updateVyGain(~,~)
  KVy=VyGain.Value;  
end
